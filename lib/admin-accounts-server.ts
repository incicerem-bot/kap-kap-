...
export type AdminAccountAction = "approve_seller" | "reject_seller" | "suspend_account" | "activate_account";
...
function safeRole(value: unknown): AdminAccountRow["role"] {
  return value === "seller" || value === "admin" ? value : "buyer";
}

function safeAccountStatus(value: unknown): AdminAccountRow["accountStatus"] {
  return value === "suspended" || value === "closed" ? value : "active";
}

function safeSellerStatus(value: unknown): AdminAccountRow["sellerStatus"] {
  return value === "pending" || value === "active" || value === "rejected" || value === "suspended" ? value : "not_started";
}

function safeReviewStatus(value: unknown): NonNullable<AdminAccountRow["seller"]>["reviewStatus"] {
...
export async function listAdminAccounts(admin: SupabaseClient, page = 1, perPage = 100, query = "") {
  ...
  const [{ data: profiles, error: profilesError }, { data: sellers, error: sellersError }] = await Promise.all([
    admin.from("kk_profiles")
      .select("id,full_name,username,role,account_status,seller_status,email_verified_at,phone_verified_at,profile_completed_at,city,last_login_at,created_at")
      .in("id", userIds),
    admin.from("kk_sellers")
      .select("id,user_id,slug,name,platform_review_status,platform_review_note")
      .in("user_id", userIds),
  ]);
  if (profilesError) throw new PaymentHttpError(503, "Kullanıcı profil şeması hazır değil.", profilesError.code);
  if (sellersError) throw new PaymentHttpError(503, "Satıcı inceleme şeması hazır değil. Paket 32 SQL dosyasını çalıştır.", sellersError.code);
  ...
  const { data: payouts, error: payoutsError } = sellerIds.length
    ? await admin.from("kk_seller_payout_accounts")
      .select("seller_id,onboarding_status,activated_at")
      .in("seller_id", sellerIds)
    : { data: [], error: null };
  if (payoutsError) throw new PaymentHttpError(503, "Satıcı ödeme durumları okunamadı.", payoutsError.code);

  const profileMap = new Map<string, ProfileDbRow>(profileRows.map((profile) => [profile.id, profile]));
  const sellerMap = new Map<string, SellerDbRow>(sellerRows.map((seller) => [seller.user_id, seller]));
  const payoutRows = (payouts ?? []) as PayoutDbRow[];
  const payoutMap = new Map<string, PayoutDbRow>(payoutRows.map((payout) => [payout.seller_id, payout]));
  ...
  }).filter((account: AdminAccountRow) => !normalizedQuery || `${account.fullName} ${account.email} ${account.username ?? ""} ${account.seller?.name ?? ""}`.toLocaleLowerCase("tr-TR").includes(normalizedQuery));
...
export async function applyAdminAccountAction(
  ...
  if (targetUserId === adminUser.id && action === "suspend_account") {
    throw new PaymentHttpError(400, "Kendi yönetici hesabını askıya alamazsın.");
  }

  const { data: profile, error: profileError } = await admin
    .from("kk_profiles")
    .select("id,role,account_status,email_verified_at,phone_verified_at,profile_completed_at")
    .eq("id", targetUserId)
    .maybeSingle();
  if (profileError || !profile) throw new PaymentHttpError(404, "Kullanıcı profili bulunamadı.", profileError?.code);
  if (action === "suspend_account" && profile.role === "admin") {
    throw new PaymentHttpError(400, "Yönetici hesapları bu ekrandan askıya alınamaz.");
  }

  const { data: seller, error: sellerError } = await admin
    .from("kk_sellers")
    .select("id,user_id,platform_review_status")
  ...
  if (action === "approve_seller" || action === "reject_seller") {
    ...
    const { data: payout, error: payoutError } = await admin
      ...
      .select("onboarding_status,submerchant_key")
      .eq("seller_id", seller.id)
      .maybeSingle();
    if (payoutError) throw new PaymentHttpError(500, "Satıcı ödeme hesabı okunamadı.", payoutError.code);

    if (action === "approve_seller") {
      if (!profile.email_verified_at || !profile.phone_verified_at || !profile.profile_completed_at) {
        throw new PaymentHttpError(422, "Satıcının e-posta, telefon ve profil doğrulamalarının tamamlanması gerekiyor.");
      }
      if (payout?.onboarding_status !== "active" || !payout.submerchant_key) {
        throw new PaymentHttpError(422, "iyzico alt üye hesabı aktif olmayan satıcı onaylanamaz.");
      }
      await admin.from("kk_sellers").update({
        platform_review_status: "approved",
        platform_review_note: reason || null,
        platform_reviewed_at: new Date().toISOString(),
        platform_reviewed_by: adminUser.id,
        is_active: true,
        verified: true,
      }).eq("id", seller.id);
      ...
      if (reason.length < 5) throw new PaymentHttpError(400, "Reddetme nedenini en az 5 karakterle yazmalısın.");
      await admin.from("kk_sellers").update({
        platform_review_status: "rejected",
        platform_review_note: reason,
        platform_reviewed_at: new Date().toISOString(),
        platform_reviewed_by: adminUser.id,
        is_active: false,
        verified: false,
    ...
    if (reason.length < 5) throw new PaymentHttpError(400, "Askıya alma nedenini en az 5 karakterle yazmalısın.");
    ...
    if (seller && seller.platform_review_status === "suspended") {
      await admin.from("kk_sellers").update({
        ...
        platform_review_note: "Hesap yeniden etkinleştirildi; mağaza tekrar inceleme kuyruğuna alındı.",
        is_active: false,
        verified: false,
      }).eq("id", seller.id);
    }
  ...
  const auditAction = action === "approve_seller" ? "seller_approved" : action === "reject_seller" ? "seller_rejected" : action === "suspend_account" ? "account_suspended" : "account_activated";
  const { error: auditError } = await admin.from("kk_admin_account_events").insert({
  ...
  if (auditError) throw new PaymentHttpError(500, "Yönetici işlem kaydı oluşturulamadı.", auditError.code);
