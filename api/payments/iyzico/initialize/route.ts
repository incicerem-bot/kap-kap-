...
export async function POST(request: NextRequest) {
  try {
    ...
    if (order.payment_status === "expired" || order.auction_offer_status === "expired" || order.status === "cancelled") {
      throw new PaymentHttpError(410, "Bu ödeme teklifinin süresi doldu. Ürün sıradaki uygun teklif sahibine sunulmuş olabilir.", "PAYMENT_WINDOW_EXPIRED");
    }
    if (auctionPaymentDeadlineExpired(order)) {
      await admin.rpc("kk_process_unpaid_auction_orders", { p_limit: 100 });
      throw new PaymentHttpError(410, "Kazanan ödeme süren doldu. Sipariş artık ödeme almıyor.", "PAYMENT_WINDOW_EXPIRED");
    }
